...
class ProductsScreenNew extends Screen
{
            ...
            Layout::table('table', [
                TD::make('id', 'id')
                    ...
                    "<img src='{$model->get('img')}'
                              alt='sample'
                              class='mw-100 d-block img-fluid rounded-1 w-100 mb-3'>
                               <span class='small text-muted mt-1 mb-0'># {$model->get('id')}</span>"),

                TD::make('name', 'Name')
                    ->width('500')
                    ->render(fn (Repository $model) =>
                    "<div class=".(($model->get('status') == "disabled")? "text-muted":"" ).">
                        <h2 class='h6 mb-3'>".Str::limit($model->get('name'),200)."</h2>
                         <p>".Str::limit($model->get('description'),200)."</p>
                         <p>{$model->get('type')}</p>
                     </div>
                     "),
